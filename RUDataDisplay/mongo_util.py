...
class MongoDB:
    ...
    def find(self, data=None, conditions=None, single=False, sort=None):
        """
        ...
        if conditions:
            for key, item in conditions.items():
                if key == "time":
                    ...
                    if item.strip():
                        data["content"] = {'$regex': re.compile(item, re.IGNORECASE)}
        # 返回列表
        if single:
            cur = [self.collection.find_one(data, {"_id": 0})]
        else:
            if sort:
                assert isinstance(sort, dict), "arg sort must be dict type, your type is {}".format(type(sort))
                assert len(sort) == 1, "sort must contain one"
                sort_key, sort_value = list(sort.items())[0]
                cur = list(self.collection.find(data, {"_id": 0}).sort(sort_key, sort_value))
            else:
...
class MongoPipline:
    ...
    def load_data_from_mongo(self, keyword=None,
                             conditions=None,
                             single=False,
                             sort=None,
                             do_filter=False):
        ...
        for key in keyword:
            self.db.change_collection(self.db_map[key])
            tmp = self.db.find(conditions=conditions, single=single, sort=sort)
